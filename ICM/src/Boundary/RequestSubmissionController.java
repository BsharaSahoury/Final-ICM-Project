...
public class RequestSubmissionController implements Initializable {
	...
	CheckBox agree;
	File fileToChoose;
   public static RequestSubmissionController ctrlSubmission;
	private static User user;
	ObservableList<String> list = FXCollections.observableArrayList("Moodle", "Student information system",
			"Lecturer information system", "Employee information system", "Library system",
			"Computers in the classroom", "Labs and computer farms", "College official site");
	...
	public void start(SplitPane splitpane, User user) {
		...
		try {
			...
			ctrlSubmission=loader.getController();
...
public  static void disablethebtn() {
	ctrlSubmission.submit.setDisable(true);
}
	public void submitRequest(ActionEvent e) {
		...
		if(ctrlSubmission.existingSituation.getText().length()>300||
				ctrlSubmission.requestedChange.getText().length()>300||
				ctrlSubmission.requestReason.getText().length()>300||
				ctrlSubmission.comment.getText().length()>300) {
				Alert alert = new Alert(AlertType.WARNING);
				alert.setTitle("TEST");
				alert.setHeaderText("Warning");
				alert.setContentText("The size of the entered text is too large");
				alert.showAndWait();
				return;
		}
		...
		Request request = new Request(chosenCombo.getSelectionModel().getSelectedItem(), existingSituation.getText(),
				requestedChange.getText(), requestReason.getText(), comment.getText(), new java.sql.Date(millis), user,
				msg, filename);
