...
public class ReportController implements Initializable {
	...
	ObservableList<String> list = FXCollections.observableArrayList("No. Active Requests", "No. Frozen Requests",
			"No. Closed Requests", "No. Rejected Requests", "No. days of Treatments");
	@FXML
	...
	public void buildGraph(ArrayList<Long> arr, Date from, Date to, String per, String rtype) {
		...
		if (rtype.contains("No. days of Treatments")) {
			...
			int i = 1;
			for (int j = 0; j < n; j++) {
				set1.getData().add(new XYChart.Data<String, Long>(String.valueOf(i) + "-" + String.valueOf(i + val - 1),
						arr.get(j)));
				i = i + val;
			}
